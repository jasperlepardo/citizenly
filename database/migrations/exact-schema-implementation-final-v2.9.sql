-- =============================================================================
-- EXACT SCHEMA IMPLEMENTATION - AUTHORITATIVE COLUMN ARRANGEMENT (FINAL)
-- =============================================================================
-- 
...
-- Note: Foreign key constraints will be added after data restoration
...
-- Restore households data first (mapping old columns to new structure, temporarily NULL household_head_id)
...
SELECT 
    code, name, address, house_number, street_id, subdivision_id,
    barangay_code, city_municipality_code, province_code, region_code,
    NULL as zip_code, -- zip_code doesn't exist in backup, set to NULL
    no_of_families, no_of_household_members, no_of_migrants,
    household_type, tenure_status, tenure_others_specify, household_unit,
    monthly_income, income_class, NULL as household_head_id, household_head_position,
    is_active, created_by, updated_by, created_at, updated_at
...
SELECT 
    ...
    NULL as occupation_code, -- occupation_code will be NULL for now
    email, mobile_number, telephone_number, household_code, blood_type,
    height, weight, complexion, citizenship, is_voter, is_resident_voter,
    last_voted_date, ethnicity, religion, religion_others_specify,
    mother_maiden_first, mother_maiden_middle, mother_maiden_last,
...
-- Update household_head_id now that residents are restored
UPDATE households 
SET household_head_id = backup.household_head_id
FROM migration_backup_households_full backup
WHERE households.code = backup.code 
AND backup.household_head_id IS NOT NULL;

-- Restore household members (mapping columns explicitly, handling missing columns)
...
-- Restore sectoral info (mapping columns explicitly, handling missing columns)
...
-- Restore relationships (mapping columns explicitly, handling missing columns)
...
SELECT 
    ...
    true as is_active, -- Default to active if column doesn't exist in backup
    COALESCE(created_at, NOW()) as created_at,
    COALESCE(updated_at, NOW()) as updated_at
FROM migration_backup_relationships;

-- =============================================================================
-- 5A. ADD FOREIGN KEY CONSTRAINTS AFTER DATA RESTORATION
-- =============================================================================

DO $$
BEGIN
    RAISE NOTICE '🔗 Adding foreign key constraints after data restoration...';
END $$;

-- Add foreign key constraints now that all data is restored
...
SELECT 
    ...
    pg_size_pretty(ts.total_size_bytes::BIGINT) AS total_size,
    pg_size_pretty(ts.index_size_bytes::BIGINT) AS index_size,
    pg_size_pretty(ts.avg_row_size_bytes::BIGINT) AS avg_row_size,
...
SELECT 
    ts.table_name,
    CASE WHEN ts.last_analyzed < NOW() - INTERVAL '7 days' THEN 'ANALYZE recommended'
         WHEN ts.row_count > 10000 AND (ts.index_size_bytes::DECIMAL / NULLIF(ts.total_size_bytes, 0)) * 100 > 50 THEN 'Review indexes - high overhead'
         WHEN ts.total_size_bytes > 1073741824 THEN 'Consider partitioning - table > 1GB'
         ELSE 'No immediate action needed' END AS recommendation,
    CASE WHEN ts.last_analyzed < NOW() - INTERVAL '7 days' THEN 'HIGH'
         WHEN ts.row_count > 10000 AND (ts.index_size_bytes::DECIMAL / NULLIF(ts.total_size_bytes, 0)) * 100 > 50 THEN 'MEDIUM'
         ELSE 'LOW' END AS priority
FROM system_table_statistics ts
WHERE CASE WHEN ts.last_analyzed < NOW() - INTERVAL '7 days' THEN 'ANALYZE recommended'
           WHEN ts.row_count > 10000 AND (ts.index_size_bytes::DECIMAL / NULLIF(ts.total_size_bytes, 0)) * 100 > 50 THEN 'Review indexes - high overhead'
           WHEN ts.total_size_bytes > 1073741824 THEN 'Consider partitioning - table > 1GB'
           ELSE 'No immediate action needed' END != 'No immediate action needed'
ORDER BY CASE WHEN ts.last_analyzed < NOW() - INTERVAL '7 days' THEN 1
              WHEN ts.row_count > 10000 AND (ts.index_size_bytes::DECIMAL / NULLIF(ts.total_size_bytes, 0)) * 100 > 50 THEN 2 ELSE 3 END;
