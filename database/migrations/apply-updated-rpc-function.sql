-- Apply the updated RPC function that matches API parameters
-- This fixes the mismatch between what the API sends and what the function accepts

-- Drop the old function first
DROP FUNCTION IF EXISTS insert_resident_encrypted CASCADE;

-- Create the updated function with all necessary parameters
...
BEGIN
    ...
    IF p_city_municipality_code IS NULL OR p_region_code IS NULL THEN
        SELECT 
            ...
            c.province_code,
            -- Derive region from province when available; otherwise use first 2 digits of city code
            COALESCE(p.region_code, SUBSTRING(b.city_municipality_code FROM 1 FOR 2))
        INTO 
            v_city_code,
            v_province_code,
            v_region_code
