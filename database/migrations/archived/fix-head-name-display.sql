-- Fix the head_name display in the view to handle null values properly
...
-- Test the fixed head name display
SELECT 
    code,
    street_name,
    house_number,
    subdivision,
    CASE 
        WHEN head_resident_first_name IS NOT NULL AND head_resident_last_name IS NOT NULL 
        THEN head_resident_first_name || ' ' || head_resident_last_name
        WHEN head_resident_first_name IS NOT NULL 
        THEN head_resident_first_name
        WHEN head_resident_last_name IS NOT NULL 
        THEN head_resident_last_name
