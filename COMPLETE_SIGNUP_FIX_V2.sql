-- Complete Signup Fix V2 - Apply remaining database schema updates
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'psgc_barangays') THEN
        ...
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.table_constraints 
            WHERE constraint_name = 'fk_barangay_accounts_barangay'
        ) THEN
...
SELECT 'barangay_accounts table structure:' as info;
SELECT 
    column_name, 
    data_type, 
    is_nullable, 
