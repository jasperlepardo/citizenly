...
export function createNumericChangeHandler<T extends Record<string, any>>(
  ...
  return (value: string | number) => {
    const numericValue = parseNumericValue(value, allowDecimals);
    const constrainedValue = applyNumericConstraints(numericValue, min, max);

    setFormData(prev => ({ ...prev, [field]: constrainedValue }));
