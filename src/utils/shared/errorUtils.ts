...
import type { AppError, ErrorLogContext, NetworkError } from '@/types/shared/errors/errors';
...
export function isValidationError(error: unknown): error is ValidationError {
  return error !== null && typeof error === 'object' && 'field' in error && 'message' in error;
}
...
export function getErrorSeverity(error: unknown): ErrorSeverity {
  ...
  if (isNetworkError(error)) {
    ...
    const status = hasStatus ? (error.status as number) : 0;
