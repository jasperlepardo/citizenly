...
import { sanitizeSearchQuery } from './sanitizationUtils';
...
export function createSuccessResponse<T>(
  ...
  const response: ApiSuccessResponse<T> = {
    ...
    message,
    meta: {
      // Add standard meta fields as needed
    },
  };
...
const databaseErrorHandlers = {
  '23505': (context?: RequestContext) => createConflictResponse('Resource already exists', context),
  '23503': (context?: RequestContext) => createErrorResponse(
    ErrorCode.VALIDATION_ERROR,
  ...
  '42P01': (context?: RequestContext) => createErrorResponse(
...
export async function handleDatabaseError(
  ...
  const handler = error?.code && databaseErrorHandlers[error.code as keyof typeof databaseErrorHandlers];
  if (handler) {
 ...
 */
async function processError(error: unknown, context?: RequestContext): Promise<Response> {
  if (errorTypeCheckers.isZodError(error)) {
  ...
  if (errorTypeCheckers.isDatabaseError(error)) {
    return handleDatabaseError(error, context);
  }

  if (errorTypeCheckers.isError(error)) {
