 ...
 * Philippine government standards and RA 10173 compliant data processing utilities.
 * Handles form validation, transformation, sanitization, and audit trail generation.
 */
...
import { sanitizeObjectByFieldTypes, sanitizeNameInput, validateNameInput } from '@/utils/auth/inputSanitizer';
...
export function transformFormData(formData: UnknownFormData): ResidentFormData {
  ...
  const sanitizedData = sanitizeObjectByFieldTypes(formData as Record<string, any>);
...
/**
 * Field validators for reducing cognitive complexity
 */
const fieldValidators: Record<string, (value: any) => string | null> = {
  email: (value: string) => VALIDATION_RULES.EMAIL_PATTERN.test(value) ? null : 'Please enter a valid email address',
  mobile_number: (value: string) => VALIDATION_RULES.PHONE_PATTERN.test(value) ? null : 'Please enter a valid Philippine mobile number (e.g., +639123456789)',
  philsys_card_number: (value: string) => VALIDATION_RULES.PHILSYS_PATTERN.test(value) ? null : 'Please enter a valid PhilSys card number (XXXX-XXXX-XXXX)',
  height: (value: number) => (value >= VALIDATION_RULES.MIN_HEIGHT && value <= VALIDATION_RULES.MAX_HEIGHT) ? null : `Height must be between ${VALIDATION_RULES.MIN_HEIGHT} and ${VALIDATION_RULES.MAX_HEIGHT} cm`,
  weight: (value: number) => (value >= VALIDATION_RULES.MIN_WEIGHT && value <= VALIDATION_RULES.MAX_WEIGHT) ? null : `Weight must be between ${VALIDATION_RULES.MIN_WEIGHT} and ${VALIDATION_RULES.MAX_WEIGHT} kg`,
};

/**
 * Validate name fields helper
 */
function validateNameFields(formData: any, errors: Record<string, string>) {
...
/**
 * Validate age from birthdate helper
 */
function validateAge(birthdate: string, errors: Record<string, string>) {
...
export function validateFormData(formData: any): ValidationResult {
  ...
  validateNameFields(formData, errors);

  // Validate age if birthdate provided
  if (formData.birthdate) {
    validateAge(formData.birthdate, errors);
  }
