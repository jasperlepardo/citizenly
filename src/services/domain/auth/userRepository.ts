...
import { validateUserData } from '../../infrastructure/validation/schemas';
import type { ValidationContext } from '../../../types/shared/validation/validation';
...
export class UserRepository extends BaseRepository<UserData> {
   ...
   */
  private applySearchFilters(query: any, options: UserSearchOptions): any {
    ...
    // Date filters
    if (options.last_login_before) query = query.lt('last_login', options.last_login_before);
    if (options.last_login_after) query = query.gt('last_login', options.last_login_after);
   ...
   */
  private applyCustomFilters(query: any, filters?: Record<string, any>): any {
  ...
  /**
   * Apply ordering and pagination to query
   */
  private applyOrderingAndPagination(query: any, options: UserSearchOptions): any {
    // Apply ordering
    if (options.orderBy) {
  ...
  async searchUsers(options: UserSearchOptions = {}): Promise<RepositoryResult<UserData[]>> {
    try {
      const queryBuilder = (supabase: SupabaseClient) => {
        ...
        query = this.applySearchFilters(query, options);
        query = this.applyCustomFilters(query, options.filters);
        query = this.applyOrderingAndPagination(query, options);
