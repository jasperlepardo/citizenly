...
import { validatePhilSysFormat } from '../../../utils/auth/sanitizationUtils';
...
export class ResidentService {
  ...
  async validateResident(formData: ResidentFormData): Promise<ResidentValidationResult> {
    try {
      ...
      if (!result.isValid) {
        ...
        const filteredErrors = errors.filter((error: { field: string; message: string }) => {
  ...
  async createResident({
    ...
    try {
      ...
      // Household head assignment is handled separately via household service
