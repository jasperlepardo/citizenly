...
import type { QueryOptions, RepositoryError, RepositoryResult } from '../../../types/infrastructure/services/services';

// Type aliases for commonly used union types
type ErrorDetails = string | Record<string, unknown> | undefined;
type SystemFields = 'id' | 'created_at' | 'updated_at';
type TimestampFields = 'created_at' | 'updated_at';
type CreateData<T> = Omit<T, SystemFields>;
type UpdateData<T> = Partial<Omit<T, 'id' | TimestampFields>>;

// Using consolidated types from @/types/infrastructure/services
...
export abstract class BaseRepository<T extends Record<string, any>> {
   ...
   * Create a new record
   */
  async create(data: CreateData<T>): Promise<RepositoryResult<T>> {
    try {
