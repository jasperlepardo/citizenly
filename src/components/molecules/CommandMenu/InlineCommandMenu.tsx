...
export function InlineCommandMenu({
  ...
  useEffect(() => {
    if (showDropdown && scrollableRef.current && filteredItems.length > 0) {
      const selectedElement = scrollableRef.current.querySelector(`[data-command-item-index="${selectedIndex}"]`);
      if (selectedElement) {
  ...
  return (
    ...
    >
      <div ref={containerRef} className="relative w-full">
        {/* Search Input */}
        <div className="flex w-full items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2.5 text-left text-sm transition-colors hover:bg-gray-50 dark:hover:bg-gray-700">
        {isLoading ? (
          <svg
            className="size-4 shrink-0 text-gray-400 dark:text-gray-500 animate-spin"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
