...
const normalizeOptions = (data: SelectOption[] | Record<string, string> | undefined): SelectOption[] => {
  if (!data) return [];
...
export default function Select({
  ...
  }, [filteredOptions, inputValue, calculateDropdownPosition, onSearch, justSelected, selectedOption]);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!onSearch) {
      ...
      const matchingOption = normalizedOptions.find(option => 
        option.label === query || option.value === query
      );
  ...
  const dropdownIcon = (
    <svg 
      className={`size-4 transition-transform duration-200 cursor-pointer ${showDropdown ? 'rotate-180' : ''}`} 
      fill="none" 
      viewBox="0 0 24 24" 
      stroke="currentColor"
    ...
    <div className="relative">
      ...
      />
        ...
        {showDropdown && (
          <div 
            ref={dropdownRef}
            className={`absolute left-0 right-0 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg z-50 max-h-60 sm:max-h-80 overflow-y-auto ${
