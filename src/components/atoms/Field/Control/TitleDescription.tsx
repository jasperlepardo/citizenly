...
const titleDescriptionVariants = cva('ml-3 flex flex-col', {
  variants: {
    size: {
      sm: 'text-sm',
      md: 'text-base',
...
const titleVariants = cva('font-medium select-none', {
  variants: {
    variant: {
...
const descriptionVariants = cva('select-none', {
  variants: {
    variant: {
...
const errorMessageVariants = cva('select-none text-red-600 dark:text-red-400', {
  variants: {
    size: {
      sm: 'text-xs',
      md: 'text-sm',
...
export const TitleDescription: React.FC<TitleDescriptionProps> = ({
  ...
  return (
    <div className={cn(titleDescriptionVariants({ size }), className)}>
      {title && (
        <span className={cn(titleVariants({ variant: actualVariant, size }))}>{title}</span>
      )}
      ...
      {errorMessage && <span className={cn(errorMessageVariants({ size }))}>{errorMessage}</span>}
    </div>
  );
};
