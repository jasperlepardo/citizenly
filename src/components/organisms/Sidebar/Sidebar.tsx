...
function SidebarHeader({ logo, title, subtitle, isCollapsed, className }: SidebarHeaderProps) {
  return (
    <div className={clsx('flex items-center px-6 py-4 border-b border-gray-200 dark:border-gray-700', className)}>
      {logo && (
...
function SidebarFooter({ children, isCollapsed, className }: SidebarFooterProps) {
  ...
  return (
    <div className={clsx('px-6 py-4 border-t border-gray-200 dark:border-gray-700 mt-auto', className)}>
...
function ToggleButton({ isCollapsed, onToggle, className }: ToggleButtonProps) {
  return (
    <button
      ...
      onClick={onToggle}
      className={clsx(
        'absolute -right-3 top-6 flex h-6 w-6 items-center justify-center rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 transition-colors z-10',
        className
      )}
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          sidebarVariants({ size: effectiveSize, position, mobile }),
          {
            ...
            'transform -translate-x-full lg:translate-x-0': !mobileOpen && mobile === 'overlay',
          },
          className
        )}
      ...
      >
        ...
        {/* Sidebar footer */}
        <SidebarFooter isCollapsed={isCollapsed && collapsible}>
          {footer}
        </SidebarFooter>
      </aside>
    </>
