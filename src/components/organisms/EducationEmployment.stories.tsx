...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'A comprehensive education and employment information form component for the RBI System. Integrates with PSOC (Philippine Standard Occupational Classification) for accurate job classification and provides complete academic and work history capture.'
      }
    }
  },
  tags: ['autodocs'],
    ...
    <div className="space-y-6">
      ...
      <div className="bg-background rounded-lg border border-default p-4">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${hasRequiredFields ? 'bg-green-500' : 'bg-red-500'}`}></span>
            <span>Required Fields: {hasRequiredFields ? 'Complete' : 'Incomplete'}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isValid ? 'bg-green-500' : 'bg-yellow-500'}`}></span>
            <span>Validation: {isValid ? 'Passed' : 'Has Errors'}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${data.psocCode ? 'bg-green-500' : 'bg-gray-400'}`}></span>
        ...
        {Object.keys(errors).length > 0 && (
          <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded">
            <h4 className="font-medium text-red-800 dark:text-red-200 text-sm mb-2">Validation Errors:</h4>
      ...
      <div className="bg-background rounded-lg border border-default p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            <strong>Education:</strong> {data.educationLevel || 'Not selected'} - {data.educationStatus || 'Not selected'}
          </div>
          <div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <h3 className="font-semibold text-blue-800 dark:text-blue-200 mb-2">PSOC Integration Features</h3>
      ...
      {data.psocCode && (
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
          <h4 className="font-medium text-green-800 dark:text-green-200 mb-2">PSOC Classification Details:</h4>
          <div className="text-sm text-green-700 dark:text-green-300 space-y-1">
            <div><strong>Code:</strong> {data.psocCode}</div>
            <div><strong>Title:</strong> {data.occupationDescription}</div>
            <div><strong>Level:</strong> {data.psocLevel.replace('_', ' ')}</div>
            <div><strong>Status:</strong> Compliant with Philippine Standard Occupational Classification</div>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
        <h3 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-2">Validation Rules Demo</h3>
