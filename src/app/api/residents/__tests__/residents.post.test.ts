...
describe('POST /api/residents', () => {
  ...
  it('returns 400 for future birthdate', async () => {
    const req = buildRequest({ firstName: 'A', lastName: 'B', birthdate: '2999-01-01', sex: 'male' });
    const res = await POST(req);
    const json = await res.json();
    expect(res.status).toBe(400);
    expect(json.error).toBe('Validation failed');
  ...
  it('returns 400 for invalid email', async () => {
    const req = buildRequest({ firstName: 'A', lastName: 'B', birthdate: '1990-01-01', sex: 'male', email: 'bad@' });
    const res = await POST(req);
    const json = await res.json();
    expect(res.status).toBe(400);
    expect(json.error).toBe('Validation failed');
  ...
  it('returns 401 for missing bearer', async () => {
    ...
    const req = new NextRequest(new Request('http://localhost/api/residents', { method: 'POST', headers, body: JSON.stringify({}) }));
    const res = await POST(req);
  ...
  it('calls RPC and returns success for minimal valid payload', async () => {
    // Mock auth success
    mockSupabaseClient.auth.getUser.mockResolvedValue({ data: { user: { id: 'u1' } }, error: null });
    ...
    const req = buildRequest({ firstName: 'A', lastName: 'B', birthdate: '1990-01-01', sex: 'male' });
    const res = await POST(req);
    const json = await res.json();
  ...
  it('maps DB error to friendly message', async () => {
    ...
    // Mock RPC error
    mockSupabaseClient.rpc.mockResolvedValue({ data: null, error: { message: 'violates row-level security policy' } });
  ...
  it('sanitizes geographic codes to exactly 10 characters', async () => {
    ...
    expect(mockSupabaseClient.rpc).toHaveBeenCalledWith('insert_resident_encrypted', expect.objectContaining({
