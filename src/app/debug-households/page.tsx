    ...
    <DashboardLayout currentPage="reports">
      <div className="p-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          ...
          <p><strong>Barangay Code:</strong> {userProfile?.barangay_code || 'Not set'}</p>
          <p><strong>User ID:</strong> {userProfile?.id || 'Not set'}</p>
        </div>

        {/* Households */}
        <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
          <h2 className="text-lg font-semibold mb-4">Households in Database ({households.length} found):</h2>
          ...
          {households.length === 0 ? (
            ...
            <div className="space-y-2">
              {households.map((household) => (
                <div key={household.id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                  <div>
                    <p><strong>Code:</strong> {household.code}</p>
                    <p><strong>ID:</strong> {household.id}</p>
                    <p><strong>Barangay:</strong> {household.barangay_code}</p>
                    <p><strong>Created:</strong> {new Date(household.created_at).toLocaleString()}</p>
                  </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <h2 className="text-lg font-semibold mb-4">Recent Residents ({residents.length} found):</h2>
            ...
            <div className="space-y-2">
              {residents.map((resident) => (
                <div key={resident.id} className="p-3 bg-gray-50 rounded">
                  ...
                  <p><strong>Household ID:</strong> {resident.household_id || 'Not set'}</p>
                  <p><strong>Household Code:</strong> {resident.household_code || 'Not set'}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mt-6">
          ...
          <ol className="list-decimal list-inside space-y-1 text-sm">
            <li>If you see households above, click "RBI Form" next to any household to test the RBI form</li>
            <li>If no households are found, you need to create some residents first (they automatically create households)</li>
            <li>Go to <a href="/residents/create" className="text-blue-600 underline">/residents/create</a> to add residents</li>
