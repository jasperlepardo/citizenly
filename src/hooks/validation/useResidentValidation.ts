...
const residentValidationSchema = z.object({
  ...
  place_of_birth: z.string().min(1, 'Place of birth is required').max(100, 'Place of birth too long'),
  ...
  // Contact Information
  contact_number: z.string().optional().refine(
  ...
  // Government IDs
  philsys_card_number: z.string().optional().refine(
 ...
 */
export const useResidentValidation = createValidationHook(
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      error.issues.forEach((err: any) => {
        ...
        errors[field].push(customErrorMessages[field as keyof typeof customErrorMessages] || err.message);
      });
