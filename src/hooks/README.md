...
const { isOpen, searchQuery, filteredItems, executeCommand } = useCommandMenuWithApi();
```
...
const { createResident, updateResident, deleteResident, isLoading } = useResidentOperations();
```
...
const { currentStep, nextStep, previousStep, submitWorkflow } = useResidentEditWorkflow();
```
...
```typescript
import { useAsyncErrorBoundary, useLogger, usePerformanceMonitor } from '@/hooks/utilities';
...
import { useOptimizedResidentValidation, useDashboard, useAsyncErrorBoundary } from '@/hooks';
```
...
function useDataWithErrorHandling() {
  ...
  const fetchData = useCallback(async () => {
    return wrapAsync(() => apiCall(), 'fetch user data');
  }, [wrapAsync]);
