...
export const personalInfoSchema = z.object({
  ...
  middleName: z.string().max(100).regex(NAME_REGEX).optional().transform(val => val ? sanitizeString(val) : undefined),
  lastName: nameSchema,
  extensionName: z.string().max(20).optional().transform(val => val ? sanitizeString(val) : undefined),
...
export const educationSchema = z.object({
  educationLevel: z.enum([
    'no_formal_education', 'elementary', 'high_school', 'college',
    'post_graduate', 'vocational', 'graduate', 'undergraduate'
  ]).optional(),
...
export const employmentSchema = z.object({
  ...
  psocCode: z.string().max(10).optional().transform(val => val ? sanitizeString(val) : undefined),
  occupationTitle: z.string().max(200).optional().transform(val => val ? sanitizeString(val) : undefined),
  workplace: z.string().max(255).optional().transform(val => val ? sanitizeString(val) : undefined)
})
...
export const healthInfoSchema = z.object({
  ...
  complexion: z.string().max(50).optional().transform(val => val ? sanitizeString(val) : undefined)
})

// Demographics Schema
export const demographicsSchema = z.object({
  ethnicity: z.enum([
    'tagalog', 'cebuano', 'ilocano', 'bisaya', 'hiligaynon',
    'bicolano', 'waray', 'kapampangan', 'pangasinan', 'maranao',
    'maguindanao', 'tausug', 'indigenous_group', 'mixed_heritage',
    'other', 'not_reported'
  ]).default('not_reported'),
  religion: z.enum([
    'roman_catholic', 'protestant', 'iglesia_ni_cristo', 'islam',
    'buddhism', 'judaism', 'hinduism', 'indigenous_beliefs',
    'other', 'none'
  ]).default('other')
})
...
export const votingInfoSchema = z.object({
  ...
  voterIdNumber: z.string().max(20).optional().transform(val => val ? sanitizeString(val) : undefined),
  lastVotedYear: z.string().regex(/^\d{4}$/, 'Year must be 4 digits').optional()
})
...
export const householdSchema = z.object({
  householdCode: z
    .string()
    .regex(HOUSEHOLD_CODE_REGEX, 'Invalid household code format')
    .optional(),
  householdNumber: z
    .string()
    .max(50)
  ...
  streetName: z.string().max(200).optional().transform(val => val ? sanitizeString(val) : undefined),
  houseNumber: z.string().max(50).optional().transform(val => val ? sanitizeString(val) : undefined),
  subdivision: z.string().max(100).optional().transform(val => val ? sanitizeString(val) : undefined),
